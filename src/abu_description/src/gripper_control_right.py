...
from pymoveit2.robots import abubot_right
...
class GripperControl(Node):
    def __init__(self):
        ...
        self._moveit2_arm = MoveIt2(
            node=self,
            joint_names=abubot_right.joint_names(),
            base_link_name=abubot_right.base_link_name(),
            end_effector_name=abubot_right.end_effector_name(),
            group_name=abubot_right.MOVE_GROUP_ARM,
            execute_via_moveit=True,
            callback_group=self._callback_group,
        )
        self._moveit2_hand = GripperInterface(
            node=self,
            gripper_joint_names=abubot_right.gripper_joint_names(),
            open_gripper_joint_positions=abubot_right.OPEN_GRIPPER_JOINT_POSITIONS,
            closed_gripper_joint_positions=abubot_right.CLOSED_GRIPPER_JOINT_POSITIONS,
            gripper_group_name=abubot_right.MOVE_GROUP_GRIPPER,
            callback_group=self._callback_group,
        )
